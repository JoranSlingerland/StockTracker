...
class DurableFunctionsHttpStartTestCase(IsolatedAsyncioTestCase):
    ...
    async def test_valid_data(self):
        ...
        with mock.patch(
            ...
            response = await main(mock_request, self.starter)
            self.assertEqual(response, mock_response)

    async def test_invalid_userid(self):
        """Test Durable Functions Http Start."""
        body, header = encode_multipart_formdata(
            {
                "functionName": "stocktracker_orchestrator",
        ...
        with mock.patch(
            ...
            response = await main(mock_request, self.starter)
            self.assertEqual(response.status_code, excpected_response.status_code)
            self.assertEqual(response.get_body(), excpected_response.get_body())
    ...
    async def test_invalid_days_to_update(self):
